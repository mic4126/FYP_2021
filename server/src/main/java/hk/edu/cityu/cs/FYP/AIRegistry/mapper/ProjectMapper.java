...
public interface ProjectMapper {
    ...
    @Select("""
            SELECT `projectId`, `projectName`, `desc`, `enabled` FROM project
            WHERE projectId = #{projectId}
            """)
    ...
    @Select("""
            SELECT `projectId`, `projectName_TC`, `desc_TC`, `enabled` FROM project
            WHERE projectId = #{projectId}
            """)
    ...
    @Select("""
            SELECT `projectId`, `projectName_SC`, `desc_SC`, `enabled` FROM project
            WHERE projectId = #{projectId}
            """)
    ...
    @Results({
            ...
            @Result(property = "projectId", column = "projectId"),
            @Result(property = "projectDesc", column = "desc"),
            @Result(property = "enabled", column = "enabled", jdbcType = JdbcType.INTEGER, javaType = Boolean.class)
    })
    @Select("""
            SELECT `projectId`, `projectName`, `desc`, `enabled` FROM project
            WHERE
    ...
    @Select("""
            SELECT `enabled` FROM `project` WHERE `projectId` = #{projectId}
            """)
